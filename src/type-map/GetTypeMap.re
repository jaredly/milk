...
let getType = (~env: Query.queryEnv, name) => {
  ...
  let%try stamp =
    ...
    |> RResult.orError(
         ...
         ++ " "
         ++ (
           Stdlib.Hashtbl.to_seq_keys(env.exported.types)->Stdlib.List.of_seq
           |> String.concat(",")
         ),
       );
  Query.hashFind(env.file.stamps.types, stamp)
  |> RResult.orError("Invalid stamp for " ++ name);
};
...
let rec getFullType_ = (env, path, name) =>
  switch (path) {
  | [] => getType(~env, name)
  | [one, ...more] =>
    let%try modStamp =
      Query.hashFind(env.exported.modules, one)
      |> RResult.orError("No exported module " ++ one);
    let%try declared =
      Query.hashFind(env.file.stamps.modules, modStamp)
      |> RResult.orError("Unexpected stamp " ++ string_of_int(modStamp));
    switch (declared.contents) {
    | Ident(_) =>
      Error("Expected a module, but " ++ one ++ " was an identifier")
    | Structure(contents) =>
...
let mapSource = (~env, ~getModule, path) => {
  ...
  let declared =
    ...
    | `Not_found =>
      print_endline(
        "Unresolved " ++ Path.name(path) ++ " in " ++ env.file.uri,
      );
      None;
    | `Stamp(stamp) =>
  ...
  | None =>
    switch (path) {
    | Path.Pident(ident) when isBuiltin(Ident.name(ident)) =>
  ...
  | Some(({name, modulePath} as declared, env)) =>
    let%try_force ((uri, moduleName), path) =
      Query.showVisibilityPath(~env, ~getModule, modulePath)
      |> RResult.orError("No visibility path");
    Public({
      uri,
      moduleName,
      declared,
  ...
  if (!Hashtbl.mem(set, key)) {
    ...
    Hashtbl.replace(
      ...
      (
        ...
        ->Belt.List.map(FromCurrent.copy_attribute),
...
let fileToReference = (~state, uri, fullName) => {
  let%try package =
    Packages.getPackage(~reportDiagnostics=(_, _) => (), uri, state);
  ...
  let%try_force moduleName =
    getModuleName(uri) |> RResult.orError("File to reference " ++ uri);
...
let forInitialType = (~tbl, ~state, uri, fullName) => {
  ...
  let moduleName =
    switch (getModuleName(uri)) {
    | None =>
      print_endline("Paths");
      package.nameForPath |> Hashtbl.iter((k, v) => {print_endline(k)});
      package.pathsForModule
      |> Hashtbl.iter((moduleName, paths) => {print_endline(moduleName)});
  ...
  Hashtbl.iter(
    (key, (attributes, value)) => {
      Hashtbl.replace(
        ...
        (
          attributes,
          SharedTypes.SimpleType.declMapSource(DigTypes.toShortSource, value),
        ),
      )
    },
    tbl,
