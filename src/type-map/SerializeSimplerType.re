...
let sourceToJson = source =>
  switch (source) {
  | DigTypes.NotFound(_) => s("NotFound")
  | Public({DigTypes.uri, modulePath, name, moduleName}) =>
    o([
      ("uri", s(uri)),
...
let rec toJson = (sourceToJson, t) =>
  ...
  | Tuple(items) =>
    l([s("Tuple"), l(items->Belt.List.map(toJson(sourceToJson)))])
  | Fn(args, result) =>
    l([
      s("Fn"),
      l(
        args->Belt.List.map(((label, arg)) =>
          o([
...
let exprToJson = (sourceToJson, body) => {
  ...
  | Record(items) =>
    l([
      s("Record"),
      o(
        items->Belt.List.map(((label, typ)) =>
          (label, toJson(sourceToJson, typ))
        ),
      ),
    ])
  | Variant(constructors) =>
    l([
      s("Variant"),
      l(
        constructors->Belt.List.map(((name, args, res)) =>
          o([
            ("name", s(name)),
...
let declToJson = (sourceToJson, {name, variables, body}) =>
  o([
    ("name", s(name)),
