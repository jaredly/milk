...
let deserialize_Migrate_parsetree__Ast_407__Parsetree__structure = json =>
  {
    let%try text = RJson.string(json);
    let lexbuf = Stdlib.Lexing.from_string(text);
    Ok(
      FromCurrent.copy_structure(Parser.implementation(Lexer.token, lexbuf)),
    );
  }
  |> wrapError;

let serialize_Migrate_parsetree__Ast_407__Parsetree__structure = structure => {
  Pprintast.structure(
    Stdlib.Format.str_formatter,
    ToCurrent.copy_structure(structure),
  );
  Json.String(Stdlib.Format.flush_str_formatter());
};

let deserialize_Migrate_parsetree__Ast_407__Parsetree__signature = json =>
  {
    let%try text = RJson.string(json);
    let lexbuf = Stdlib.Lexing.from_string(text);
...
let serialize_Migrate_parsetree__Ast_407__Parsetree__signature = data => {
  Pprintast.signature(
    Stdlib.Format.str_formatter,
    ToCurrent.copy_signature(data),
  );
  Json.String(Stdlib.Format.flush_str_formatter());
};

let deserialize_Migrate_parsetree__Ast_407__Parsetree__pattern = json =>
  {
    let%try text = RJson.string(json);
    let lexbuf = Stdlib.Lexing.from_string(text);
...
let serialize_Migrate_parsetree__Ast_407__Parsetree__pattern = data => {
  Pprintast.pattern(
    Stdlib.Format.str_formatter,
    ToCurrent.copy_pattern(data),
  );
  Json.String(Stdlib.Format.flush_str_formatter());
};

let deserialize_Migrate_parsetree__Ast_407__Parsetree__core_type =
    (json)
    : result(Migrate_parsetree.Ast_407.Parsetree.core_type, list(string)) =>
  {
    let%try text = RJson.string(json);
    let lexbuf = Stdlib.Lexing.from_string(text);
...
let serialize_Migrate_parsetree__Ast_407__Parsetree__core_type = data => {
  Pprintast.core_type(
    Stdlib.Format.str_formatter,
    ToCurrent.copy_core_type(data),
  );
  Json.String(Stdlib.Format.flush_str_formatter());
};

let deserialize_Migrate_parsetree__Ast_407__Parsetree__expression = json =>
  {
    let%try text = RJson.string(json);
    let lexbuf = Stdlib.Lexing.from_string(text);
    Ok(
      FromCurrent.copy_expression(
...
let serialize_Migrate_parsetree__Ast_407__Parsetree__expression = data => {
  Pprintast.expression(
    Stdlib.Format.str_formatter,
    ToCurrent.copy_expression(data),
  );
  Json.String(Stdlib.Format.flush_str_formatter());
};
...
let migrate_Stdlib__hashtbl__t = (keyMigrator, valueMigrator, data) => {
  ...
  data
  |> Stdlib.Hashtbl.iter((key, value) =>
       Hashtbl.replace(result, keyMigrator(key), valueMigrator(value))
     );
  result;
};
...
let serialize_Belt__Belt_HashMapInt____t = (_valueTransform, _map) => {
  Json./* map->Belt.HashMap.Int.toArray; */
       Null;
...
let serialize_Stdlib__hashtbl____t = (keyTransform, valueTransform, tbl) => {
  Vendor.Json.Array(
    Hashtbl.fold(
      (key, value, result) => {
        [
          Vendor.Json.Array([keyTransform(key), valueTransform(value)]),
          ...result,
        ]
      },
      tbl,
