...
let failer = message => {
  [%expr (_) => print_endline([%e stringConst(message)])];
};
  ...
  MakeSerializer.{
    wrapWithVersion: [%expr
      (version, payload) => {
        ...
        | JSONObject(dict) =>
          Js.Dict.set(dict, schemaPropertyName, Js.Json.number(float_of_int(version)));
...
let deserializeTransformer = {
  ...
  parseVersion: [%expr 
    json => switch (Js.Json.classify(json)) {
      | JSONObject(dict) => switch (Js.Dict.get(dict, schemaPropertyName)) {
  ...
  variant: (~renames, constructors) => {
    let cases = constructors->Belt.List.map(((name, argCount, argTransformer)) => {
      ...
      Exp.case(
        pat,
        ~guard=[%expr Js.Json.JSONString([%e stringConst(MakeDeserializer.getRename(~renames, name))]) == Js.Json.classify(tag)],
        argTransformer
      )
    });
