...
let rx = Str.regexp("[^a-zA-Z_]");

let makeLockedTypeName = (moduleName, modulePath, name) => {
  String.concat(
    "__",
    ["_" ++ Str.global_replace(rx, "_", moduleName)] @ modulePath @ [name],
  );
};

let unflatten = items =>
  switch (Longident.unflatten(items)) {
  | None => assert(false)
  | Some(lident) => lident
  };
...
let rec outputDeclaration =
        (
          ...
          name,
          showSource,
          declaration: SharedTypes.SimpleType.declaration('source),
        ) => {
  let declarationName: Ast_helper.str =
    Location.mknoloc(makeLockedTypeName(moduleName, modulePath, name));
  let fullReference =
    switch (alias) {
    | None => None
    | Some(reference) =>
      Some(
        Ast_helper.Typ.constr(
          Location.mknoloc(reference),
          declaration.variables->Belt.List.map(outputExpr(showSource)),
        ),
      )
    };
  ...
  let mk =
    Ast_helper.Type.mk(
      ~params=
        ...
        ->Belt.List.map(expr =>
            (outputExpr(showSource, expr), Asttypes.Invariant)
          ),
    );
            ...
            Ast_helper.Type.constructor(
              ~args=
                Parsetree.Pcstr_tuple(
                  contents->List.map(outputExpr(showSource)),
                ),
...
and outputExpr =
    (~mapVariable=name => Ast_helper.Typ.var(name), showSource, expr) => {
  Ast_helper.Typ.(
    switch (expr) {
    ...
    | RowVariant(rows, closed) =>
      ...
      variant(
        rows->Belt.List.map(((label, expr)) => {
          Parsetree.Rtag(
            Location.mknoloc(label),
            [],
            false,
            switch (expr) {
            | None => []
    ...
    | Reference(source, args) =>
      showSource(
        source,
        args->List.map(outputExpr(~mapVariable, showSource)),
      )
    | Tuple(items) =>
      tuple(items->List.map(outputExpr(~mapVariable, showSource)))
    | Fn(args, result) =>
      let rec loop = args =>
        switch (args) {
        ...
        | [(_label, arg), ...rest] =>
          arrow(
            Nolabel,
            outputExpr(~mapVariable, showSource, arg),
            loop(rest),
          )
        };
      loop(args);
