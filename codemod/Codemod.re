...
let run = (~rootPath, ~filterPath, modify) => {
  let mainFile =
    Utils.startsWith(rootPath, ".")
      ? Filename.concat(Sys.getcwd(), rootPath) : rootPath;
  ...
  let%try_force package =
    Analyze.Packages.getPackage(
      ~reportDiagnostics=(_, _) => (),
      mainFileUri,
      state,
    );
  ...
  let fullForCmt =
    (
      switch (package.compilerVersion) {
  ...
  package.Analyze.TopTypes.localModules
  ->Belt.List.forEach(moduleName => {
      let%opt_force paths =
        Utils.maybeHash(package.pathsForModule, moduleName);
